...
1. 모든 filesystem 접근은 `PathGuard`(`resolveExisting` / `resolveForWrite`)를 통과한다. tool이나 filesystem 모듈에서 입력 path로 직접 절대 경로를 조립하지 않는다.
...
8. 새 tool은 `runTool`로 감싸고(timeout, error 변환, audit) `annotations`를 설정한다. shell, Git, process 실행 tool은 별도 ADR 없이 추가하지 않는다.
